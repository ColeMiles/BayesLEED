...
CVB  Include global parameters for dimension statements etc.
      ...
      DIMENSION ATSMK(NBTD,NDATT),PQ(2,NBTD),ESMK(NDATT)
...
C  PERFORM DOMAIN-AVERAGING
      CALL RINTAV(ATSMK,NSS,NBTD,NETI,PQ,KAV,SYM,NBT,ESMK,IPR)
C  CHECK FOR TOO HIGH THEOR. INTENS.             
      DO 28 IB=1,NBT                             
      CALL MAXINT(ATSMK,NSS,NBTD,IB,NETI,AM,NDATT)
...
40    NET(IB)=NETI
      ...
      CALL STRIP2(ATSMK,NSS,NBTD,NET,IS,AT,NBT,ESMK,ET,NDATT)
...
C  HAS BEEN EXPLOITED, OTHERWISE ALL WEIGHTS ARE 1.)
      SUBROUTINE RINTAV(AT,NS,NB,NE,PQ,KAV,SYM,LAVM,ES,NPRIN)            281279
...
C     WRITE(6,30)IS,(PQ(1,IB),PQ(2,IB),IB=1,LAVM)
  30  FORMAT(1H ,//,19H0SURFACE STRUCTURE ,I3,//,45H BEAMS AND BEAM INTE
     1NSITIES AFTER AVERAGING  ,5(/,22X,8(1X,2A6),/,28X,8(1X,2A6)))
      DO 32 IE=1,NE
  32  WRITE(6,35)ES(IE),(AT(IS,IB,IE),IB=1,LAVM)
  35  FORMAT(10H ENERGY = ,1F7.2,5H EV  ,8E13.5,5(/,28X,8E13.5,/,22X,
     18E13.5))
  40  CONTINUE
      RETURN
      END
